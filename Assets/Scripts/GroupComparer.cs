...
public class GroupComparer : IComparer<Group>
{
    ...
    {
        //in a group 2^(number of set bits in the direction) gives the number of singles in a group
        //therefore greater the set bits, greater the number of singles in the group, greater the group
        if (Kmap.countSetBits(x.direction) > Kmap.countSetBits(y.direction))
        {
            return 1;
        }
        else if (Kmap.countSetBits(x.direction) < Kmap.countSetBits(y.direction))
        {
            return -1;
        }
        else
